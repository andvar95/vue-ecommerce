<template>
  <div class="single-product-container">
    <div class="product-img">
      <img
        src="https://image.made-in-china.com/202f0j10SmRaBwqoSbrO/5-2-Inch-CDMA-Smart-Phone-Cheap-Cell-Phone.jpg"
        alt="s"
      />
    </div>
    ...
    <div v-if="name" class="product-description">
      ...
      <div class="product-cart">
        <input v-model="cartQuantity" class="number-poducts" type="number" />
        <button v-on:click="addProduct()" class="cart-button">
        ...
        await this.$apollo
          .mutate({
            mutation: gql`
              mutation Mutation(
                $addCartId: String!
                $addCartInputDetailProducts: InputDetailProducts!
              ) {
                addCart(
                  id: $addCartId
                  inputDetailProducts: $addCartInputDetailProducts
                ) {
                  userId
                  orderId
                  date
