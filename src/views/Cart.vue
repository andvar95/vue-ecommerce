<template>
  <div class="cart-container pad-1">
    <div class="cart">
      <h2 class="cart-title">
        <div>Carrito</div>
        <div v-if="errorStockMsg" class="errorStockMsg">Error en el Stock</div>
      </h2>

      <cart-card
        v-for="(cart, key) in orderByUserAndStatus"
        :key="key"
        :userId="cart.userId"
...
export default {
  ...
  apollo: {
    orderByUserAndStatus: {
      query: gql`
        query(
          $orderByUserAndStatusId: String
          $orderByUserAndStatusStatus: String
        ) {
          orderByUserAndStatus(
            id: $orderByUserAndStatusId
            status: $orderByUserAndStatusStatus
          ) {
            userId
            orderId
            date
  ...
  methods: {
    ...
    editCart(data) {
      const edited = addToCart(
        data,
        JSON.parse(JSON.stringify(this.orderByUserAndStatus[0]))
      );
